...
if __name__ == '__main__':
    # 构建模型
    model = load_model('xina.h5') # 加载训练模型
    wavs=[]
    wavs.append(get_wav_mfcc("D:\\c++\\pythonProject\\sound\\xina.wav"))
    X=np.array(wavs)
    print(X.shape)
    result=model.predict(X[0:1])[0] # 识别出第一张图的结果，多张图的时候，把后面的[0] 去掉，返回的就是多张图结果
    print("识别结果",result)
    #  因为在训练的时候，标签集的名字 为：  0：xina   1：stop    0 和 1 是下标
    name = ["西娜","stop"] # 创建一个跟训练时一样的标签集
    ind=0 # 结果中最大的一个数
