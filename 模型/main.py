...
def create_datasets():
    ...
    for i in files:
        ...
        # print(waveData)
        wavs.append(waveData)
        if ("西娜" in labsInd)==False:
            labsInd.append("西娜")
        labels.append(labsInd.index("西娜"))
    ...
    for i in files:
        ...
        waveData = get_wav_mfcc(path+i)
        testwavs.append(waveData)
        if ("西娜" in testlabsInd)==False:
            testlabsInd.append("西娜")
        testlabels.append(testlabsInd.index("西娜"))
...
if __name__ == '__main__':
    ...
    # 构建模型
    model = Sequential()
    model.add(Dense(256, activation='relu',input_shape=(16000,)))
    ...
    print('Test accuracy:', score[1]) # 准确
    # 度

    model.save('xina.h5') # 保存训练模型